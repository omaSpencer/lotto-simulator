...
import { DRAW_NUMBERS_COUNT, DRAWS_PER_YEAR, MAX_DRAWS, TICKET_PRICE_HUF, } from './src/lib/constants';
...
app.prepare().then(() => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('start-simulation', (data) => {
            ...
            if (Array.isArray(data?.playerNumbers) &&
        ...
        const loop = async () => {
            while (isRunning && drawCount < MAX_DRAWS) {
                ...
                if (matchCount === DRAW_NUMBERS_COUNT)
                    break;
